 ...
 */
#ifndef BASEADDON_INCL
#define BASEADDON_INCL
...
// This class doesn't have much in it yet, but collects all Base classes that are Addons
class BaseAddon : public Addon {
    JBALLOC_NO_DESTRUCTOR_(BaseAddon)

public:
    DYNAMIC_ALLOC_ONLY(BaseAddon, BaseExtension *bx, Extensible *root, KINDTYPE(Extensible)=KIND(Extensible));

protected:
    BaseExtension *bx() const { return static_cast<BaseExtension *>(ext()); }

    SUBCLASS_KINDSERVICE_DECL(Extensible, BaseAddon);
};
