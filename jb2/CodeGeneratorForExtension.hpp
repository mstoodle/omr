...
#define MISSING_CG_OP_HANDLER(C,name) \
    Builder * \
    C::gencode ## name(Operation *op) { \
        return missingCodeGeneratorOperation(LOC, op); \
    }

class CodeGeneratorForExtension : public Extensible {
...
public:
    ...
    virtual bool registerContext(Context *c);
    virtual bool registerLiteral(Literal *lv);
    virtual bool registerScope(Scope *s);
    virtual bool registerSymbol(Symbol *sym);
    virtual bool registerType(const Type *type);
    virtual bool registerValue(Value *value);

protected:
    CodeGenerator *cg() const;
    Builder *missingCodeGeneratorOperation(LOCATION, Operation *op);
