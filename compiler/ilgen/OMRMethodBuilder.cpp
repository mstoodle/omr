...
// Note: _memoryRegion and the corresponding TR::SegmentAllocator and TR::Memory instances are stored as pointers within MethodBuilder
...
OMR::MethodBuilder::MemoryManager::MemoryManager() :
   _backingMemoryAllocator(new (TR::Compiler->persistentAllocator()) TR::BackingMemoryAllocator(TR::Compiler->rawAllocator, MEM_SEGMENT_SIZE, 0)),
   _segmentAllocator(new(TR::Compiler->persistentAllocator()) TR::SegmentAllocator(*_backingMemoryAllocator, MEM_SEGMENT_SIZE)),
...
OMR::MethodBuilder::MemoryManager::~MemoryManager()
   ...
   static_cast<TR::SegmentAllocator *>(_segmentAllocator)->~SegmentAllocator();
   ::operator delete(_segmentAllocator, TR::Compiler->persistentAllocator());
   static_cast<TR::BackingMemoryAllocator *>(_backingMemoryAllocator)->~BackingMemoryAllocator();
