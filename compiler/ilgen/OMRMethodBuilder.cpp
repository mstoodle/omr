...
OMR::MethodBuilder::connectTrees()
   ...
   if (_useBytecodeBuilders)
      ...
      // following is neede3d by JB2 to ensure all builders are appropriately counted */
      ListIterator<TR::BytecodeBuilder> iter(_allBytecodeBuilders);
      for (TR::BytecodeBuilder *builder=iter.getFirst();
           !iter.atEnd();
           builder = iter.getNext())
...
OMR::MethodBuilder::adjustNameForInlinedSite(const char *name)
   ...
   size_t len = (4+10+1+1+strlen(name)) * sizeof(char); // 4 ("_INL") + max 10 digits + 1 ("_") + original name string + trailing zero
   char *newName = (char *) _comp->trMemory()->allocateHeapMemory(len);
   snprintf(newName, len, "_INL%u_%s", _inlineSiteIndex, name);
...
OMR::MethodBuilder::DefineFunction(const char* const name,
   ...
   TR::DataType *methodParmTypes = (TR::DataType *) trMemory()->heapMemoryRegion().allocate(numParms * sizeof(TR::IlType *));
   for (int32_t p=0;p < numParms;p++)
      {
...
OMR::MethodBuilder::Compile(void **entry)
   ...
   TR::DataType *methodParmTypes = (TR::DataType *) trMemory()->heapMemoryRegion().allocate(_numParameters * sizeof(TR::IlType *));
   const char **methodParmNames = (const char **) trMemory()->heapMemoryRegion().allocate(_numParameters * sizeof (char *));
   for (int32_t p=0;p < _numParameters;p++)
      {
   ...
   TR::ResolvedMethod resolvedMethod(getDefiningFile(),
                                     getDefiningLine(),
                                     GetMethodName(),
                                     getNumParameters(),
                                     methodParmNames,
