...
public:
   ...
   MemorySegment(RawSegment const segment, size_t const size) :
      _segment(segment),
      _size(size),
      _allocated(0),
      _next(this)
   ...
   MemorySegment(const TR::MemorySegment &other) :
   ...
   RawSegment base() const throw()
      {
      return _segment;
      }

   // Note that bytes parameter is used as-is; it is not rounded to any alignment
   RawSegment allocate(size_t bytes)
      {
      // seems strong to assert if there's no space, but TR::MemorySegments are not used directly by most of the JIT code base
   ...
   // rewind() should ONLY be used if allocate() succeeds but for some reason the memory area cannot be used
   void rewind(size_t bytes)
      {
      TR_ASSERT(_allocated > bytes, "Error: rewind() amount should not exceed allocated bytes");
      _allocated -= bytes;
      }

   // Bulk free all earlier allocations from this segment
   void reset() throw()
      {
      _allocated = 0;
      }
...
private:
   ...
   size_t _size;
   size_t _allocated;
   MemorySegment *_next;   // sadly, since class not yet fully defined, no way to use reference_wrapper
   };
