...
static const OptimizationStrategy partialRedundancyEliminationOpts[] =
   ...
   { OMR::localCSE,                     }, // cleanup before strider so it will not be confused by commoned nodes (mandatory to run local CSE before strider)
   { OMR::deadTreesElimination,         }, // cleanup before strider so that dead stores can be eliminated more effcientlly (i.e. false uses are not seen)
   { OMR::loopStrider,                  OMR::IfLoops  },
...
static const OptimizationStrategy methodHandleInvokeInliningOpts[] =
   ...
   { OMR::localValuePropagation            }, // Propagate known-object info and derive more specific archetype specimen symbols for inlining
#ifdef J9_PROJECT_SPECIFIC
   { OMR::targetedInlining,                },
#endif
   { OMR::deadTreesElimination             },
   { OMR::methodHandleInvokeInliningGroup, OMR::IfEnabled }, // Repeat as required to inline all the MethodHandle.invoke calls we can afford
   { OMR::endGroup                         },
   };
...
static const OptimizationStrategy ilgenStrategyOpts[] =
   ...
   { OMR::invariantArgumentPreexistence, OMR::IfNotClassLoadPhaseAndNotProfiling }, // Should not run if a recompilation is possible
#endif
   { OMR::endOpts                        },
   };
...
void OMR::FullOptimizer::enableAllLocalOpts()
   {
   this->TR::MinOptimizer::enableAllLocalOpts();
